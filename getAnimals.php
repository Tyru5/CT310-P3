...
function create_list( status ){
  ...
  for(var i in status){
    ...
    str +='<li>' + nameOfDog +status[i].petName + '</li>' +
          ...
          '<li><a href= animal_view.php?id=' + status[i].petId + '&imageURL=' + status[i].imageURL + '&descURL=' + status[i].descURL + '&petName=' + status[i].petName + '&petKind=' + status[i].petKind '>Click here to view Pet!</a></li><br>';
        }
  }
  str += '</ul>';
